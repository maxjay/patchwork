...
// Returned by get(path, { key }) and getBase(path, { key }). Same shape as
// the plain get result but with identity extracted from the key field,
// mirroring the identity field on DiffOp add/remove ops.
export type GetWithKeyResult<U extends JsonValue = JsonValue> = {
	path: string;
	value: U;
	identity?: JsonValue;
};
...
export class Engine<T extends JsonValue = JsonValue> {
	...
	//
	// When options.key is supplied, each result also carries an identity field
	// extracted from that key — mirroring the identity on DiffOp add/remove ops.
	// Use '$self' for primitive arrays (same semantics as diff).
	get<U extends JsonValue = JsonValue>(jsonPath: string, options: { key: string }): Array<GetWithKeyResult<U>>;
	get<U extends JsonValue = JsonValue>(jsonPath: string): Array<{ path: string; value: U }>;
	get(jsonPath: string, options?: { key?: string }): Array<{ path: string; value: JsonValue; identity?: JsonValue }> {
		const matched = paths(this.draft, jsonPath);
		const results = matched.map(p => ({ path: p, value: this.getAt(this.segmentsFrom(p)) }));
		if (!options?.key) return results;
		return results.map(r => {
			const id = this.extractIdentity(r.value, options.key!);
			return id !== undefined ? { ...r, identity: id } : r;
		});
	}

	// Same as get() but reads from base instead of draft.
	getBase<U extends JsonValue = JsonValue>(jsonPath: string, options: { key: string }): Array<GetWithKeyResult<U>>;
	getBase<U extends JsonValue = JsonValue>(jsonPath: string): Array<{ path: string; value: U }>;
	getBase(jsonPath: string, options?: { key?: string }): Array<{ path: string; value: JsonValue; identity?: JsonValue }> {
		const matched = paths(this.base, jsonPath);
		const results = matched.map(p => ({ path: p, value: this.getAt(this.segmentsFrom(p), this.base) }));
		if (!options?.key) return results;
		return results.map(r => {
			const id = this.extractIdentity(r.value, options.key!);
			return id !== undefined ? { ...r, identity: id } : r;
...
export class NodeEngine<T extends JsonValue = JsonValue> {
	...
	get(jsonPath: string, options?: { key?: string }): Array<any> {
		const results = this.parent.get(joinPath(this.prefix, jsonPath), options as any);
		return results.map(r => ({ ...r, path: rebasePath(r.path, this.prefix) }));
	}

	getBase<U extends JsonValue = JsonValue>(jsonPath: string, options: { key: string }): Array<GetWithKeyResult<U>>;
	getBase<U extends JsonValue = JsonValue>(jsonPath: string): Array<{ path: string; value: U }>;
	getBase(jsonPath: string, options?: { key?: string }): Array<any> {
		const results = this.parent.getBase(joinPath(this.prefix, jsonPath), options as any);
		return results.map(r => ({ ...r, path: rebasePath(r.path, this.prefix) }));
	}
